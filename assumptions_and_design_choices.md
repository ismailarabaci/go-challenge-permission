...
**Unit Tests:** 24 tests organized by stage (`Test_Stage{X}_{Description}`), table-driven where applicable  
**Integration Tests:** 2 end-to-end HTTP tests with 11 subtests covering complex permission scenarios  
**Go Patterns Used:** Table-driven tests, subtests with `t.Run()`, test helpers with `t.Helper()`, `httptest.Server`

### Rationale

**Benefits:** Each test covers specific scenario, table-driven tests simplify adding cases, descriptive names are self-documenting, helper functions eliminate duplication

**Why HTTP in tests:** Validates realistic usage through HTTP layer without requiring production HTTP code

### Conclusion
All tests passing in ~2 seconds with comprehensive coverage of user operations, groups, hierarchies, transitive membership, and permissions.
...
**Four jobs** run in parallel on every push/PR:
1. **Unit Tests:** Go 1.20 with MySQL 8.0 service container, race detector, coverage reporting to Codecov
2. **Integration Tests:** End-to-end HTTP scenarios with MySQL 8.0 service container, race detector, coverage reporting
3. **Lint:** golangci-lint with comprehensive linters (errcheck, gosec, govet, staticcheck, gofmt, ineffassign, dupl)
4. **Format:** Code style enforcement with `gofmt` and `go vet`

**Features:** Separate test jobs for unit and integration tests, MySQL service containers mirror production, Go module caching speeds builds (~2-3 min runs), Dependabot for automated updates
...
**Simpler Queries:** No type discriminators needed. `SELECT name FROM users WHERE id = ?` vs `SELECT name FROM principals WHERE id = ? AND type = 'user'`

**Database-Level Constraints:** Foreign keys enforce business rules impossible to violate at application level

**Performance:** Smaller tables, more efficient indexes, better query optimization without type filters

### Conclusion
Separate tables leverage database constraints for business rule enforcement, simpler queries, and accurate domain modeling.
